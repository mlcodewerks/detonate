...
bool    ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    ...
    // Failed shader creation must release partially-created objects and
    // restore the frontend's pixel-unpack binding too.
    auto fail = [&](GLuint vertex, GLuint fragment)
    {
    ...
    if (!CheckShader(frag_handle, "fragment shader"))
        return fail(vert_handle, frag_handle);
...
// Libretro may report an already-lost context. Discard CPU bookkeeping
// without issuing GL calls against names that may now belong to the frontend.
void ImGui_ImplOpenGL3_AbandonContext()
{
    ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
    if (!bd) return;
    for (ImTextureData* tex : ImGui::GetPlatformIO().Textures)
