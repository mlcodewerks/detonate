...
{
    struct Stream {
        const uint8_t* bytes; size_t size, pos = 0;
        size_t Read(void* dst, size_t n) {
            if (n > size-pos) throw std::runtime_error("Truncated Organya song.");
            memcpy(dst, bytes+pos, n); pos += n; return n;
        }
        void Seek(int, int) { pos = 0; }
        enum { kSeekBegin = 0 };
    };

    //========= PART 0 : INPUT/OUTPUT AND UTILITY ========//

    int coggetc(Stream* fp) {
        uint8_t value;
    ...
    int coggetw(Stream* fp) { int a = coggetc(fp); int b = coggetc(fp); return (b<<8) + a; }
    int coggetd(Stream* fp) { int a = coggetw(fp); int b = coggetw(fp); return (b<<16) + a; }
    ...
    struct Song {
        ...
        bool Load(Stream* fp) {
            ...
            loop_end        = coggetd(fp);
            if (ms_per_beat <= 0 || loop_start < 0 || loop_end <= loop_start || uint64_t(ms_per_beat) * loop_end > 3600000) return false;
    ...
    Song* Load(const uint8_t* bytes, size_t size, uint32_t sampleRate) {
        Stream input{bytes, size}; auto* stream = &input;
        auto m_song = new Organya::Song{};
        bool loaded;
        try { loaded = m_song->Load(stream); } catch (...) { delete m_song; throw; }
        if(!loaded) {
            delete m_song;
            return nullptr;
        }
