...
struct hvl_tune *hvl_load_ahx( const uint8 *buf, uint32 buflen, uint32 defstereo, uint32 freq, hvl_malloc_callback mallocCb, hvl_free_callback freeCb )
{
  const uint8  *bptr;
  const TEXT   *nptr;
  ...
  snprintf(ht->ht_Name, sizeof(ht->ht_Name), "%s", (TEXT *)&buf[(buf[4]<<8)|buf[5]]);
...
struct hvl_tune *hvl_load_hvl( const uint8 *buf, uint32 buflen, uint32 defstereo, uint32 freq, hvl_malloc_callback mallocCb, hvl_free_callback freeCb )
{
  const uint8  *bptr;
  const TEXT   *nptr;
...
struct hvl_tune *hvl_ParseTune( const uint8 *buf, uint32 buflen, uint32 freq, uint32 defstereo, hvl_malloc_callback mallocCb, hvl_free_callback freeCb)
{
	...
	if( ( buf[0] == 'T' ) &&
	   ...
	   ( buf[3] < 3 ) )
	{
		ht = hvl_load_ahx( buf, buflen, defstereo, freq, mallocCb, freeCb );
	}
			...
			( buf[3] < 2 ) )
	{
		ht = hvl_load_hvl( buf, buflen, defstereo, freq,  mallocCb, freeCb );
	}
	else {
		printf( "Invalid file.\n" );
	}
...
struct hvl_tune *hvl_LoadTune( const TEXT *name, uint32 freq, uint32 defstereo, hvl_malloc_callback mallocCb, hvl_free_callback freeCb )
{
  struct hvl_tune *ht = NULL;
  uint8  *buf;
  uint32  buflen;
  ...
  ht = hvl_ParseTune( buf, buflen, freq, defstereo, mallocCb, freeCb );
  freeCb( buf );
  return ht;
}
...
void hvl_Decode( struct hvl_tune *ht, int16 *buf, uint32 samples )
{
  hvl_play_irq(ht);
  hvl_mixchunk(ht, samples, (int8*)buf, (int8*)(buf + 1), 4);
}

uint32 hvl_GetDuration( struct hvl_tune *ht )
{
  uint32 duration = 0;
  do
  {
...
uint32 hvl_Seek(struct hvl_tune* ht, uint32 timeInMs)
{
    uint32 PosNr, i;
    ...
    uint32 duration = 0;
    uint32 end = timeInMs * ht->ht_SpeedMultiplier * 50 / 1000;
    do
    {
        hvl_play_irq(ht);
        duration++;
...
void hvl_UpdateStereo(struct hvl_tune* ht, uint32 stereoAsPercentage)
{
    if (ht->ht_isAHX)
    {
        const int32 defgain[] = { 71 * 256 / 100, 72 * 256 / 100, 76 * 256 / 100, 85 * 256 / 100, 100 * 256 / 100};
        uint32 i;

        if (stereoAsPercentage >= 100)
        {
            ht->ht_defpanleft = stereopan_left[4];
            ht->ht_defpanright = stereopan_right[4];
            ht->ht_mixgain = defgain[4];
        }
        else
        {
            int32 defstereo = 4 * stereoAsPercentage / 100;
            int32 factor = stereoAsPercentage % 25;

            ht->ht_defpanleft = stereopan_left[defstereo] + (stereopan_left[defstereo + 1] - stereopan_left[defstereo]) * factor / 25;
            ht->ht_defpanright = stereopan_right[defstereo] + (stereopan_right[defstereo + 1] - stereopan_right[defstereo]) * factor / 25;
            ht->ht_mixgain = defgain[defstereo] + (defgain[defstereo + 1] - defgain[defstereo]) * factor / 25;
        }
