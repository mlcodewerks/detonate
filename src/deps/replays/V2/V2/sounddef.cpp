...
void sdInit()
{
    ...
    for (int i = 0; i < 129; i++)
    {
        if (i < 128)
        {
            patchoffsets[i] = (int32_t)(sptr - soundmem);
            snprintf(s, sizeof(s), "Init Patch #%03d", i);
            snprintf(patchnames[i], sizeof(patchnames[i]), "%s", s);
        } else
            editmem = sptr;
    ...
    snprintf(v2clipname, sizeof(v2clipname), "Init Patch");
...
//ATLASSERT(v2vsizes[v2version]==v2nparms);
    ...
    snprintf(speech[0], sizeof(speech[0]), "%s", "!DHAX_ !kwIH_k !br4AH_UHn !fAA_ks !jAH_mps !OW!vER_ !DHAX_ !lEY!zIY_ !dAA_g ");
#endif
}
...
void sdCopyPatch()
{
    ...
    snprintf(v2clipname, sizeof(v2clipname), "%s", patchnames[v2curpatch]);
}

void sdPastePatch()
{
    memcpy(soundmem + 128*4 + v2curpatch*v2soundsize, v2clipboard, v2soundsize);
    snprintf(patchnames[v2curpatch], sizeof(patchnames[v2curpatch]), "%s", v2clipname);
}

void sdInitPatch()
{
    memcpy(soundmem + 128*4 + v2curpatch*v2soundsize, v2initsnd, v2soundsize);
    snprintf(patchnames[v2curpatch], sizeof(patchnames[v2curpatch]), "Init Patch #%03d", v2curpatch);
}
