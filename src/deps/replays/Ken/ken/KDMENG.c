...
int32_t scale (int64_t a, int32_t d, int32_t c) {
	return (a * d)/c;
}
...
unsigned char trinst[MAXTRACKS], trquant[MAXTRACKS];
unsigned char trvol1[MAXTRACKS], trvol2[MAXTRACKS];
int nttime[MAXNOTES];
unsigned char nttrack[MAXNOTES], ntfreq[MAXNOTES];
unsigned char ntvol1[MAXNOTES], ntvol2[MAXNOTES];
unsigned char ntfrqeff[MAXNOTES], ntvoleff[MAXNOTES], ntpaneff[MAXNOTES];
...
short coef[NUMCOEF<<LCOEFPREC]; //Sound re-scale filter coefficients
...
int stereohicomb (int dummy, int *voloffs, int samps, int sinc, int splc, int *stemp)
{
	unsigned char *ucptr;
	...
	for(i=0;i<samps;i++)
	{
		...
		splc = splc % kdmasm3 - kdmasm3; // Wrap even if a high pitch skips multiple loops.
	}
	return(splc);
}

void bound2short (int bytespertic, int *stemp, short *sptr)
{
	int i, j;
...
inline void fsin (int *a) { (*a) = (int)(sin((float)(*a)*(PI*1024.f))*16384.f); }

inline void calcvolookupstereo (int *lut, int p0, int i0, int p1, int i1)
{
	int i;
	for(i=0;i<256;i++) { lut[i*2] = p0; lut[i*2+1] = p1; p0 += i0; p1 += i1; }
}

inline int mulscale16 (int a, int d) { return (int)((int64_t(a)*d)>>16); }
inline int mulscale24 (int a, int d) { return (int)((int64_t(a)*d)>>24); }
inline int mulscale30 (int a, int d) { return (int)((int64_t(a)*d)>>30); }
inline void clearbuf (int *d, int c, int a) { for(c--;c>=0;c--) d[c] = a; }
inline void copybuf (int *s, int *d, int c) { for(c--;c>=0;c--) d[c] = s[c]; }
...
int loadwaves (Loader* loader)
{
	...
	if (!(snd = (unsigned char *)malloc(totsndbytes+2))) goto fail;
...
int kdmload (Loader* loader)
{
	...
	if (!kdminited)
	{
		...
		{
			char tbuf[2048];
			if (strlen(loader->GetName(loader)) >= sizeof(tbuf)-16) return ERR_CODE;
...
int32_t kdmrendersound (char *dasnd, int numbytes)
{
...
void kdmseek (int32_t seektoms)	// resurrected from old version
{
	int32_t i;
