...
inline int32_t scale (int32_t a, int32_t d, int32_t c)
{
	return a * d /c;
}
...
int32_t smsndsamplerate, smsndnumspeakers = 2, smsndbytespersample = 2;


char chantrack[18];
int32_t nownote, waitcnt, startsnd = 27, loopcnt;
int32_t speed = 240, nspeed, musicstatus = 0, minicnt;
unsigned char chords[1500][13], inster[3], drumstat;	// EMSCRIPTEN changed to unsigned
int32_t freq[63] =
{
	0,
...
char inst[30][11] =					// EMSCRIPTEN seems this is actually signed
{
...
void noteoff (int32_t chan, int32_t freq)
{
...
void noteon (int32_t chan, int32_t freq)
{
	if (chan >= 9) chan += 256-9;
	adlib0(0xa0+chan,0);
	adlib0(0xb0+chan,0);
...
void setinst (int32_t chan, char v0, char v1, char v2, char v3, char v4, char v5,
...
void drum (int32_t drumnum, int32_t freq)
{
	int32_t chan;
...
int watrand () { watrind++; return(wrand[watrind-1]); }
void randoinsts ()
{
	int32_t i, j, k;
	float f;
...
void smnexttic ()
{
	int32_t i, j, k, templong;
	char ch, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10;

	waitcnt--; if (waitcnt > 0) return;

	if (nownote + 1 >= chord) { nownote = -1; loopcnt++; }
	nownote++;
...
int32_t smsndload (Loader* loader)
{
	int32_t i, j, k, l;
	unsigned char buffer[15], instr[12];
	...
	for(i=j=0;i<chord;i++) j += (int32_t)chords[i][12];
	return(scale(j,1000,nspeed));
}

int32_t smsndrendersound (void *dasnd, int32_t numbytestoprocess)
{
	int32_t i, prepcnt;
...
void smsndseek (int32_t seektoms)
{
	int32_t i, j, k, l, templong;
	...
	while ((j < i) && (nownote < chord))
		{ j += (int32_t)chords[nownote][12]; nownote++; }
	if (nownote + 1 >= chord) nownote = 0;
