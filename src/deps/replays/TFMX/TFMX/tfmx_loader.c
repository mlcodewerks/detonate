...
static int tfmx_loader(TfmxState* state, TfmxData* mdat_data, TfmxData* smpl_data) {
    ...
    if (read_data(&state->mdat_header, sizeof(state->mdat_header), mdat_data) != sizeof(state->mdat_header)) {
    ...
    if (!(x = read_data(&state->mdat_editbuf, (MDAT_EDITBUF_LONGS - 1) * 4, mdat_data))) {
    ...
    if (state->mdat_header.trackstart >= (U32)state->mlen ||
        state->mdat_header.pattstart >= (U32)state->mlen ||
        state->mdat_header.macrostart >= (U32)state->mlen) return -1;
    ...
    for (x = 0; x < 128 && z < state->mlen; x++) {
        y = (_byteswap_ulong(state->mdat_editbuf[z]) - 0x200);
        if ((y & 3) || (y < 0 || (y >> 2) >= state->mlen)) /* probably not strictly right */
            break;
    ...
    for (x = 0; x < 128 && z < state->mlen; x++) {
        y = (_byteswap_ulong(state->mdat_editbuf[z]) - 0x200);
        if ((y & 3) || (y < 0 || (y >> 2) >= state->mlen))
            break;
        state->mdat_editbuf[z++] = y >> 2;
    }
    state->num_pat = x;

    if (!state->num_pat || !state->num_mac || state->patterns[0] < (int)state->mdat_header.trackstart) return -1;
...
*/
    ...
    if (!smpl_data->size || !(state->smplbuf = (void*)malloc(smpl_data->size))) {
    ...
    if ((buf[10] == 'b' && buf[11] == 'y') ||
        ...
        if (buf[464] == 0x00 && buf[465] == 0x00 && buf[466] == 0x00 && buf[467] == 0x00) {
            if ((buf[14] != 0x0e && buf[15] != 0x60) || /*z-out title */
                (buf[14] == 0x08 && buf[15] == 0x60 && mdat_data->size > 4645 && buf[4644] != 0x09 && buf[4645] != 0x0c) || /* metal law */
                (buf[14] == 0x0b && buf[15] == 0x20 && mdat_data->size > 5121 && buf[5120] != 0x8c && buf[5121] != 0x26) || /* bug bomber */
                (buf[14] == 0x09 && buf[15] == 0x20 && mdat_data->size > 3977 && buf[3876] != 0x93 && buf[3977] != 0x05)) { /* metal preview */
                state->isPro = 0;
            }
        }
    }
