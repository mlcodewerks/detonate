...
loop:
    for (a = 0; a < stackSize; a += 2)
    {
        if (stack[a] == c->MacroPtr && stack[a + 1] == c->MacroStep)
            return;// endless loop
    }
    if (stackSize == 512)
        return;
    stack[stackSize++] = c->MacroPtr;
    stack[stackSize++] = c->MacroStep;
    x.l = _byteswap_ulong(state->mdat_editbuf[c->MacroPtr + (c->MacroStep++)]);
    a = x.b.b0;
    x.b.b0 = 0;
    DEBUG(3) {
...
static void player_DoMacro(TfmxState* state, int cc) {
    ...
    c->hw->vol = (U16)c->CurVol * (U16)state->mdb.MasterVol; // don't clamp the volume to improve quality
}
    ...
    if ((l[0]) == 0xEFFE) {
        switch (l[1]) {
            ...
            case 1: /* loop */
                ...
                if ((state->mdb.TrackLoop < 0 || state->pdb.CurrPos == state->pdb.LastPos) && state->loops > 0) { // infinite loop (Masterblazer race)
                    if (!(--state->loops)) {
                        state->mdb.PlayerEnable = 0;
                        return;
                    }
...
static int player_DoTrack(TfmxState* state, struct Pdb* p, int pp) {
    ...
    while (1) {
    loop:
        x.l = _byteswap_ulong(state->mdat_editbuf[p->PAddr + p->PStep++]);
        t = x.b.b0;
    ...
    if (!state->mdb.SpeedCnt--) {
        ...
        for (i = 0; i < 8; i++) {
            ...
            if (player_DoTrack(state, &state->pdb.p[i], i)) {
                emptyTracks = endlessLoop = 0;
                i = -1;
                continue;
            }
            if (state->pdb.p[i].PAddr == 0 || state->pdb.p[i].PNum >= 0x90)
            {
                emptyTracks |= 1 << i;
                endlessLoop |= 1 << i;
            }
            else if (state->pdb.p[i].isLoop)
                endlessLoop |= 1 << i;
        }
        if (emptyTracks == 0xff)
        {
...
void TFMXRewind(TfmxState* state) {
    ...
    state->hdb[0].vol = 0x40 * 64; // don't clamp the volume to improve quality
    state->hdb[0].mode = 3;
...
U32 player_tfmxGetDuration(TfmxState* state)
{
    uint64_t duration = 0;
    S32 nb = 0;
    ...
    return (U32)((1000 * duration) / state->outRate);
}
